...
const WF_ID = 'Mp3kMd5Je7JVek2O' // GM_Dashboard_auth_delivery_worker
...
const backupPath = `scripts/backups/${WF_ID}_${stamp}.json`
writeFileSync(backupPath, JSON.stringify(current, null, 2))
console.log(`Backup → ${backupPath}`)

// Reuse the Postgres credential from the existing node
const oldPg = current.nodes.find(n => n.type === 'n8n-nodes-base.postgres')
const pgCreds = oldPg?.credentials || {}
if (!pgCreds.postgres) {
  console.error('Could not find postgres credentials on existing node — aborting')
  process.exit(1)
}
...
WHERE outbound_message_id = '{{ $('Pick & lock email queue').item.json.outbound_message_id }}'::uuid;`
...
const updated = await n8n('PUT', `/workflows/${WF_ID}`, putBody)
